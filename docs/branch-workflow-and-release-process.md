...
1. Create a `release/x.y.z-hotfix` branch from `main`. The branch name must still start with `release/` so the `auto-version-bump` and `auto-release` workflows apply.
