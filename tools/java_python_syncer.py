...
class JavaPythonSyncer:
    ...
    def _to_snake_case(self, camel_str):
        ...
        """Create a GitHub issue for sync tasks with completion tracking"""
        python_changes = self.get_python_changes()
        total_items = len(sync_items)
        completed_items = sum(1 for item in sync_items if item["python_file"] in python_changes)
        
        issue_body = f"## Java to Python Sync Required ({completed_items}/{total_items} completed)\n\n"
        issue_body += "The following Java files have been updated and require " \
                      "corresponding Python changes:\n\n"

        for item in sync_items:
            recently_modified = item["python_file"] in python_changes
            if recently_modified:
                checkbox = "- [x]"
                status = "🔄 UPDATED"
            elif item["exists"]:
            ...
            issue_body += f"{checkbox} **{status}**: `{item['python_file']}`\n"
            issue_body += f"  - Based on: `{item['java_file']}`\n\n"

        if completed_items > 0:
            issue_body += f"\n✅ **Progress**: {completed_items} of {total_items} items completed\n"

        issue_body += "\n---\n*This issue was automatically generated by the Java-Python sync tool.*"

        # Use GitHub CLI to create issue
        try:
            subprocess.run([
                'gh', 'issue', 'create',
                '--title', f'Python Sync Required: {completed_items}/{total_items} completed',
                '--body', issue_body,
                '--label', 'python-sync-needed'
